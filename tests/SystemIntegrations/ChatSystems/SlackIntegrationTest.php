...
class SlackIntegrationTest extends TestCase
{
    ...
    {
        $tickets = [new TicketInfo("FOO-0", 'Goto google.com', 'https://google.com')];
        self::assertTrue(self::$slack->isWithinRateLimit($tickets));

        for ($i = 1; $i < 10000; $i++)
        {
            $tickets[] = new TicketInfo("FOO-$i", 'Goto google.com', 'https://google.com');
        }
        self::assertFalse(self::$slack->isWithinRateLimit($tickets));
    }
    ...
    public function testMessageTruncating () : void
    {
        ...
        $message = self::$slack->getChatMessage($tickets, "nowhere", "nothing");
